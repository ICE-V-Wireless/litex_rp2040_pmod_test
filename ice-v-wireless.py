...
from litex_boards.targets.ice_v_wireless import _CRG

# IOs ----------------------------------------------------------------------------------------------

def rp2040_spi_ios(pmod="PMOD2"):
    return [
        ("rp2040_spi", 0,
            Subsignal("clk",  Pins(f"{pmod}:3")), # SPI1 SCK.
            Subsignal("cs_n", Pins(f"{pmod}:4")), # SPI1 CSn.
            Subsignal("mosi", Pins(f"{pmod}:7")), # SPI1 TX.
            Subsignal("miso", Pins(f"{pmod}:2")), # SPI1 RX.
            IOStandard("LVCMOS33"),
        )
    ]
...
class RP2040PMODTestSoC(SoCCore):
    def __init__(self, sys_clk_freq=int(50e6), mode="litex"):
        platform = ice_v_wireless.Platform()
        platform.add_extension(rp2040_spi_ios(pmod="PMOD2"))
        ...
        SoCMini.__init__(self, platform, sys_clk_freq,
            ident         = "QWERTY RP2040 PMOD <==> LiteX SoC proof of concept on ICE-V Wireless.",
            ident_version = True
        )
